...
<div class="max-w-2xl mx-auto">
    <div class="bg-[#1a120f] border border-[#33231c] rounded-2xl p-6 shadow-sm">
        ...
        <div class="flex items-center justify-between pb-4 mb-4 border-b border-[#33231c]">
            <div class="flex items-center space-x-2">
                ...
                <h3 class="font-serif-title font-bold text-base text-[#fffdfa]">Edit Kategori: {{ $category->name }}</h3>
            </div>

            <a href="{{ route('admin.products.index', ['category_id' => $category->id]) }}" class="text-xs text-[#e2c159] hover:underline flex items-center space-x-1">
                <span>{{ $category->products_count }} Produk di kategori ini</span>
        ...
        <form action="{{ route('admin.categories.update', $category->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4 text-xs">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label class="block font-semibold text-stone-300 mb-1">Slug URL</label>
                    <input
                        type="text"
                        name="slug"
                        value="{{ old('slug', $category->slug) }}"
                        class="w-full p-2.5 bg-[#261914] text-white rounded-xl border border-[#4a342b] focus:border-[#c89e2b] outline-none font-mono">
                    <p class="text-[10px] text-stone-400 mt-1">Identitas unik untuk URL & filter kategori.</p>
                </div>
                <div>
                    ...
                    <input
                        ...
                        min="0"
                        class="w-full p-2.5 bg-[#261914] text-white rounded-xl border border-[#4a342b] focus:border-[#c89e2b] outline-none">
                    <p class="text-[10px] text-stone-400 mt-1">Urutan posisi kategori di Beranda & Menu.</p>
                </div>
            </div>
            ...
            <div>
                ...
                <textarea
                    name="description"
                    rows="3"
                    class="w-full p-2.5 bg-[#261914] text-white rounded-xl border border-[#4a342b] focus:border-[#c89e2b] outline-none">{{ old('description', $category->description) }}</textarea>
            </div>

            <!-- Current Image & Photo Upload -->
            <div>
                <label class="block font-semibold text-stone-300 mb-1">Foto / Banner Kategori Saat Ini</label>
                <div class="flex items-center space-x-4 mb-2">
                    <div class="w-20 h-14 rounded-xl overflow-hidden bg-[#261914] border border-[#382621] flex-shrink-0">
                        <img src="{{ $category->image_url }}" alt="{{ $category->name }}" class="w-full h-full object-cover">
                    </div>
                    <span class="text-[11px] text-stone-400">Pilih file baru di bawah ini jika ingin mengganti gambar banner kategori.</span>
                </div>
                <input
                    type="file"
                    name="image"
